...
> **Anonymity notice.** This repository has been prepared for double-blind review at **NeurReps 2026** (Symmetry and Geometry in Neural Representations, a NeurIPS workshop). All author names, affiliations, and identifying contact details have been removed from this README, `CITATION.cff`, and `LICENSE`. One thing anonymizing *content* can't fix: the *account* hosting this repository. If this is pushed under your own GitHub username or organization, that alone identifies you to a reviewer who looks. Consider an anonymous GitHub account, or mirroring through a double-blind hosting service (e.g. `anonymous.4open.science`) until decisions are out.
...
```mermaid
flowchart TD
    Start(["Network trained on a<br/>group-structured task"])
    Start --> Split{"Which symmetry group?"}

    Split -->|"Modular arithmetic<br/>ℤₚ, cyclic"| ModStruct["Every non-trivial irrep<br/>is 2-dimensional"]
    Split -->|"Sparse parity<br/>(ℤ/2)ⁿ, hypercube"| ParStruct["Every irrep is 1-dimensional<br/>(Walsh characters),<br/>combinatorially numerous"]

    ModStruct --> ModFew["Task needs only a<br/>few active frequencies"]
    ParStruct --> ParHard["SQ-hardness (Barak et al. 2022):<br/>optimizer can't consolidate onto<br/>a small subset of directions"]

    ModFew --> Lemma["Lemma 1 -- isotypic rank bound<br/><i>(proven)</i><br/>rank &le; dimension of occupied<br/>isotypic components"]
    ParHard --> Conj["Conjecture -- parity shouldn't<br/>collapse<br/><i>(explicit hypothesis, tested,<br/>not a theorem)</i>"]

    Lemma --> ModCollapse["Rank collapses<br/>&#10003; useful precursor<br/>r = 0.87, p &lt; 10&#8315;&#8308;"]
    Conj --> ParHigh["Rank predicted to stay high"]
    ParHigh --> ParTest["Confirmed empirically<br/>&#10007; precursor fails"]
...
```bibtex
@inproceedings{anonymous2026isotypic,
...
Withheld for double-blind review. Author correspondence during the review period should go through the venue's official reviewing platform, not this repository.
